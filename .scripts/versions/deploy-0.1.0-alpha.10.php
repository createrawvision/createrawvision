...
function deploy_nav_menus() {
	...
	foreach ( array( $main_menu_id, $main_menu_member_id ) as $menu_id ) :
		...
		$entry_menu_item_id = run_wp_cli_command( "menu item add-custom $menu_id 'Neu hier?' '' --porcelain", array( 'return' => 'stdout' ) );
		run_wp_cli_command( "menu item add-post $menu_id 19838 --parent-id=$entry_menu_item_id" ); // 'Neu hier?'
		...
		$intro_menu_item_id = run_wp_cli_command( "menu item add-custom $menu_id 'Was ist Rohkost?' '' --porcelain", array( 'return' => 'stdout' ) );
		run_wp_cli_command( "menu item add-post $menu_id 12777 --parent-id=$intro_menu_item_id" ); // 'Was ist Rohkost?'
		run_wp_cli_command( "menu item add-post $menu_id 19475 --parent-id=$intro_menu_item_id" ); // 'Was versteht man unter veganer Rohkost?'
		run_wp_cli_command( "menu item add-post $menu_id 19361 --parent-id=$intro_menu_item_id" ); // 'Welche Rohkost Richtungen gibt es?'

		if ( $menu_id === $main_menu_member_id ) {
			run_wp_cli_command( "menu item add-term $menu_id category 5869 --title=Rohkost&nbsp;Rezepte" );
		} else {
			$recipes_menu_item_id = run_wp_cli_command( "menu item add-custom $menu_id 'Rohkost&nbsp;Rezepte' '' --porcelain", array( 'return' => 'stdout' ) );
			run_wp_cli_command( "menu item add-term $menu_id category 5869 --title=Alle&nbsp;Rezepte --parent-id=$recipes_menu_item_id" );
		...
		/** @todo add community (forum, q&a, events) */
	...
	run_wp_cli_command( "menu item add-custom $secondary_menu_id '$facebook_svg' 'https://www.facebook.com/Create-Raw-Vision-596361277187093/' --target=_blank" );
	run_wp_cli_command( "menu item add-custom $secondary_menu_id '$pinterest_svg' 'https://de.pinterest.com/CreateRawVision' --target=_blank" );
	run_wp_cli_command( "menu item add-custom $secondary_menu_id '$youtube_svg' 'https://www.youtube.com/channel/UCDn-CVZvNd6xqXM0g1Zu4pg' --target=_blank" );
	run_wp_cli_command( "menu item add-custom $secondary_menu_id '$instagram_svg' 'https://www.instagram.com/createrawvision/' --target=_blank" );
