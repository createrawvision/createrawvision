...
add_action('rcp_user_profile_updated', function ($user_id, $userdata, $old_userdata) {
...
function crv_add_user_to_mailchimp($new_email, $first_name, $last_name, $old_email = '')
{
    ...
    try {
        ...
        $MailChimp->put("lists/{$list_id}/members/{$subscriber_hash}", [
...
function crv_update_rcp_status_mailchimp($old_status, $new_status, $membership_id)
{
    ...
    try {
        ...
        $tag_data = ['email_address' => $user->user_email];
...
add_action('rcp_email_settings', function ($rcp_options) { ?>
