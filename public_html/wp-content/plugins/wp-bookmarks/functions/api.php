...
class wpb_api {
	...
	function post_thumb( $postid, $size = 400 ) {
		include_once wpb_path . 'lib/BFI_Thumb.php';
	...
	function public_bookmarks( $args = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			$output .= '<p>' . sprintf( __( 'You need to <a href="%1$s">login</a> or <a href="%2$s">register</a> to view and manage your bookmarks.', 'wpb' ), wp_login_url( get_permalink() ), site_url( '/wp-login.php?action=register&redirect_to=' . get_permalink() ) ) . '</p>';
		...
		if ( is_user_logged_in() ) {
			...
			foreach ( $collections as $id => $array ) {
				if ( ! isset( $array['label'] ) ) {
					$array          = array();
					$array['label'] = $default_collection;
				}
				if ( $id === $active_coll ) {
					$class = 'active';
				} else {
	...
	function bookmarkpopup( $postid, $args = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			$output .= '<p>' . sprintf( __( 'You need to <a href="%1$s">login</a> or <a href="%2$s">register</a> to bookmark/favorite this content.', 'wpb' ), wp_login_url( get_permalink() ), site_url( '/wp-login.php?action=register&redirect_to=' . get_permalink() ) ) . '</p>';
